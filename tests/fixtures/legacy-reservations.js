...
function extractReservations() {
  ...
  // Procesar nuevas reservaciones
  newThreads.forEach(thread => {
    const messages = thread.getMessages();
    messages.forEach(message => {
      if (processMessage(sheet, message, 'Confirmada')) {
        newReservations++;
      }
    });
  });
  ...
  modThreads.forEach(thread => {
    const messages = thread.getMessages();
    messages.forEach(message => {
      if (processMessage(sheet, message, 'Modificada')) {
        modifications++;
      }
    });
  });
  ...
  cancelThreads.forEach(thread => {
    const messages = thread.getMessages();
    messages.forEach(message => {
      if (processMessage(sheet, message, 'Cancelada')) {
        cancellations++;
      }
    });
  });
  ...
  const guestyResult = extractGuestyReservations();
 ...
 */
function processMessage(sheet, message, status) {
  const messageId = message.getId();

  // Verificar si este correo ya fue procesado
  if (isAlreadyProcessed(sheet, messageId)) {
  ...
  if (reservationData) {
    addToSheet(sheet, reservationData);
...
function isAlreadyProcessed(sheet, messageId) {
  ...
  // Verificar si el messageId existe comparando el código de confirmación y fecha
  for (let i = 1; i < data.length; i++) {
    if (data[i][6] === messageId) { // Columna Confirmation Code
      return true;
    }
  }
 ...
 */
function findReservationRow(sheet, confirmationCode) {
  const data = sheet.getDataRange().getValues();

  // Buscar por Confirmation Code (columna 7, índice 6)
  for (let i = 1; i < data.length; i++) {
    if (data[i][6] === confirmationCode) {
...
function addToSheet(sheet, data) {
  ...
  const existingRow = findReservationRow(sheet, data.confirmationCode);
 ...
 */
function extractGuestyReservations() {
    ...
    messages.forEach(message => {
      ...
      reservations.forEach(reservation => {
        if (reservation.confirmationCode) {
          const result = processGuestyReservation(sheet, reservation);
 ...
 */
function processGuestyReservation(sheet, guestyData) {
